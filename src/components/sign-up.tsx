...
export const SignUpPage: React.FC<SignUpPageProps> = ({
  ...
  description = "Join our workspace platform to optimize your daily focus engines",
  heroImageSrc,
  testimonials = [],
  children,
}) => {
  return (
    <div className="h-[100dvh] flex flex-col md:flex-row font-geist w-[100dvw] overflow-hidden">
      {/* ── Left pane ── h-full keeps it inside the 100dvh outer wrapper */}
      <section className="flex-1 flex items-center justify-center p-6 overflow-y-auto h-full">
        <div className="w-full max-w-md">
          <div className="flex flex-col gap-3">
            <h1 className="animate-element animate-delay-100 text-3xl md:text-4xl font-semibold leading-tight">{title}</h1>
            <p className="animate-element animate-delay-200 text-muted-foreground text-sm">{description}</p>

            <div className="animate-element animate-delay-300 w-full">
              {children}
            </div>
          </div>
        </div>
